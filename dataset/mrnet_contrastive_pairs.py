...
PREPROCESSING_TRANSORMS = tio.Compose([
    tio.RescaleIntensity(out_min_max=(-1, 1), include = keys),
    tio.CropOrPad(target_shape=(32, 256, 256), include = keys)
])


TRAIN_TRANSFORMS = tio.Compose([
    tio.RandomAffine(degrees=(5,5,5),p=0.5,scales=0,default_pad_value='minimum', include = ["data1"]),
    tio.RandomFlip(axes=(2), flip_probability=0.5, include = ["data1"]),
])




class MRNetDatasetContrastivePairs(Dataset):
    def __init__(self, root_dir, task, plane, split='train', preprocessing_transforms=None, transforms=None, fold=0, augmentation = True):
        super().__init__()
        self.task = task
        self.plane = plane
    ...
    def __getitem__(self, index, Augprob = 1):
        ...
        data2 = torch.from_numpy(data.copy())

        sample = {'data1': data1, 'data2':data2}
        sample = self.preprocessing_transforms(sample)  
