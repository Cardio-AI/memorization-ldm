...
class Unet3D(nn.Module):
    def __init__(
        ...
        # text conditioning
        print(type(cond_dim));print( cond_dim is None);print(cond_dim)
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                ...
                if exists(self.cfg.model.cond_dim): 
                    cond = data['cond'][:, np.nonzero(self.cfg.model.cond_sel)[0]].cuda()
                else:
                    cond = None    
                data = data['data'].cuda()
                #start = time.process_time()
                ...
                with torch.no_grad():
                    ...
                    if exists(self.cfg.model.cond_dim): 
                        all_videos_list = list(
                            map(lambda n: self.ema_model.sample(batch_size=n, cond = cond[[0],:]), batches))
                    else:
                        all_videos_list = list(
